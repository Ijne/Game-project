...
class Building:
    ...
    def build(self, type, position, view, camera):
        global all_stick_walls, x, y, location

        if type == 'stick':
            print(position[0] * int(x), position[1] * int(y))
            if view == 0:
                if position[1] != 29 and position[1] != 0:
                    position[1] -= 1
                    if isinstance(board.field[position[0]][position[1]], Sticks) is False and \
                            isinstance(board.field[position[0]][position[1]], Stones) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Brown_Stones) is False and \
                            isinstance(board.field[position[0]][position[1]], Grass) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Brown_Grass) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Sticks_Wall) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Stone_Wall) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Dark_grass) is False and \
                            isinstance(board.field[position[0]][position[1]], Honey) is False and \
                            isinstance(board.field[position[0]][position[1]], Carrot) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Creator) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Berries) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Blue_fire) is False and \
                            isinstance(board.field[position[0]][position[1]], NPS_1) is False and \
                            isinstance(board.field[position[0]][position[1]], NPS_2) is False and \
                            isinstance(board.field[position[0]][position[1]],
                                       Mushroom) is False and \
                            location != 'dark-forest':
